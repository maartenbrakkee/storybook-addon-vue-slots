...
// src/utils.ts
var wrappedTemplate = (template, slotName) => {
  const templateWithDefault = template ?? `{{ args.${slotName} }}`;
  const wrap2 = (contents2) => `<template #${slotName}>
${contents2}
</template>`;
  const wrappedTemplate2 = templateWithDefault.includes(`<template #${slotName}`) ? templateWithDefault : wrap2(templateWithDefault);
  return wrappedTemplate2.replace(/<template #default>(\n*)(.*)(\n*)<\/template>/gimsu, "$2");
};
var SLOTS_CATEGORY_NAME = "slots";

// src/render.ts
function toKebabCase(inputString) {
  return inputString.replace(/([A-Z])([A-Z])/g, "$1-$2").replace(/([a-z])([A-Z])/g, "$1-$2").replace(/[\s_]+/g, "-").toLowerCase();
}
function getArgString(key, value) {
...
var renderWithSlots = () => {
  const makeComponentTemplate = (component, slots, args) => {
  ...
  return (args, { viewMode, componentId, component, parameters }) => {
    ...
    if (!parameters?.slots) {
      return {
        ...
        setup: () => ({ args: vue.computed(() => ({ ...args })) })
      };
    }
    const slots = Object.entries(parameters.slots).reduce((acc, [currentSlotName, params]) => `${acc}
${wrappedTemplate(typeof params === "object" ? params.template : void 0, currentSlotName)}`, "");
...
var render_default = renderWithSlots;
...
function compileToken(token, options, context) {
  ...
  const query = token.map((rules) => {
    if (rules.length >= 2) {
      ...
      if (first2.type !== SelectorType.Pseudo || first2.name !== "scope") ; else if (isArrayContext && second.type === SelectorType.Descendant) {
...
var Tokenizer2 = class {
  ...
  stateBeforeClosingTagName(c) {
    if (isWhitespace3(c)) ; else if (c === CharCodes2.Gt) {
      this.state = State2.Text;
    } else {
...
// src/source.ts
var makeDefaultSlots = (slotNames) => slotNames.reduce((acc, name) => ({ ...acc, [name]: `{{ args.${name} }}` }), {});
var source_default = (code, { component, argTypes, parameters }) => {
  const $2 = load(code, {
    xml: {
      xmlMode: true,
  ...
  const componentName = component?.__name || component.name;
  const componentEl = $2(componentName);
  const slots = Object.keys(argTypes).filter((key) => argTypes[key].table?.category === SLOTS_CATEGORY_NAME);
  const templates = Object.entries(parameters.slots || {}).reduce((acc, [key, val2]) => ({ ...acc, [key]: typeof val2 === "object" && val2.template !== void 0 ? val2.template : `{{ args.${key} }}` }), {});
  const slotArgs = getSlotArgs(code);
  const slotTemplates = {
    ...makeDefaultSlots(slots),
    ...templates
  };
  const children2 = componentEl.contents().filter((i, el) => el.type === "tag" && el.name === "template" || el.type === "text");
  children2.remove();
...
var renderArgs = (template, args) => {
  const handlebarsRegex = /\{\{\s*'(.*)'\s*\}\}/gmiu;
  const propRegex = /:(.*)="'(.*)'"/gmiu;
  const withArgs = Object.entries(args).reduce((acc, [key, val2]) => acc.replaceAll(`args.${key}`, `'${val2}'`), template);
...
var getSlotArgs = (code) => {
  const slotRegex = /<template #(.*)>"(.*)"<\/template>/gmiu;
  const matches = [...code.matchAll(slotRegex)];
  const args = matches.reduce((acc, [_, slotName, arg]) => ({ ...acc, [slotName]: arg }), {});
  if (args.default === void 0) {
...
// src/preview.ts
var withSlots = (story, { component, parameters }) => {
...
var convertSlotArgTypes = (context) => {
  if (!context.parameters.slots)
